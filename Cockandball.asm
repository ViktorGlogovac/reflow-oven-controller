...
PWM_OUTPUT    equ P1.1 ; Attach an LED (with 1k resistor in series) to P1.0
...
state2message: 	  db 'SoakS 2 T        ', 0
state3message: 	  db 'RtoP3 T         ', 0
state4message: 	  db 'Reflow 4 T      ', 0
state5message: 	  db 'Cooling 5 T     ', 0
state1messagea:   db 'RamptoSoak C    ', 0
state2messagea:   db 'SoakS 2 C      ', 0
state3messagea:   db 'RtoP3 C     ', 0
state4messagea:   db 'Reflow 4 C      ', 0
state5messagea:   db 'Cooling 5 C     ', 0
blank:            db '                ', 0
yusuf:            db 'YUSUF           ', 0
...
Display_BCD(seconds)
Set_Cursor(2, 1)
Send_Constant_String(#state2messagea)
display_temp(x)
...
FSM1_state4_b1:
	ljmp FSM1_state4
FSM1_state3:
	cjne a, #3, FSM1_state4_b1
	Set_Cursor(1, 1)
	Send_Constant_String(#state3message)
	Set_Cursor(2, 1)
	Send_Constant_String(#state3messagea)
	display_temp(x)
    mov pwm_ratio+0, #low(1000)
    mov pwm_ratio+1, #high(1000)
    mov a, temp_refl
...
FSM1_state5_b1:
	ljmp FSM1_state5
FSM1_state4:
	cjne a, #4, FSM1_state5_b1
	Set_Cursor(1, 1)
	Send_Constant_String(#state4message)
	Set_Cursor(2, 1)
	Send_Constant_String(#state4messagea)
	display_temp(x)
    mov pwm_ratio+0, #low(200)
    mov pwm_ratio+1, #high(200)
    mov a, Time_refl
...
FSM1_state5:
	...
	Set_Cursor(2, 1)
	Send_Constant_String(#state5messagea)
	display_temp(x)
